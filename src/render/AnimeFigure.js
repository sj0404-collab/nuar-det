...
export function buildAnimeEyes(group, { cx = 0, cy = 0, cz = 0.12, dist = 0.115, radius = 0.05, iris = 0x2a7f8f, width = 0.028 } = {}) {
  ...
  for (const s of [-1, 1]) {
    ...
    const white = new THREE.Mesh(flatGeometry(new THREE.SphereGeometry(radius, 6, 5)), whiteMat);
    white.scale.set(1, 0.72, 0.5);
    white.position.set(cx + s * dist, cy, cz);
    // big iris disc pushed toward the camera
    const irisMesh = new THREE.Mesh(flatGeometry(new THREE.SphereGeometry(radius * 0.66, 6, 5)), irisMat);
    irisMesh.scale.set(1, 1.15, 0.42);
    irisMesh.position.set(cx + s * dist, cy, cz + radius * 0.5);
    // pupil + highlight
    const pupil = new THREE.Mesh(flatGeometry(new THREE.SphereGeometry(radius * 0.3, 6, 4)), pupilMat);
    pupil.scale.set(1, 1.1, 0.5);
    pupil.position.set(cx + s * dist, cy, cz + radius * 0.72);
    const glint = new THREE.Mesh(new THREE.SphereGeometry(radius * 0.14, 5, 4), glintMat);
...
export function buildAnimeHair(group, { cx = 0, cy = 0, cz = 0, radius = 0.15, color = 0x2a2118, style = 'spiky', tilt = 0 } = {}) {
  ...
  const cap = new THREE.Mesh(flatGeometry(new THREE.SphereGeometry(radius * 1.02, 7, 5)), hairMat);
  ...
  if (style === 'spiky') {
    for (let i = 0; i < 4; i++) {
      const tuft = new THREE.Mesh(flatGeometry(new THREE.ConeGeometry(radius * 0.16, radius * 0.55, 4)), hairMat);
    ...
    for (const s of [-1, 1]) {
      const side = new THREE.Mesh(flatGeometry(new THREE.SphereGeometry(radius * 0.42, 6, 4)), hairMat);
      side.scale.set(0.8, 1.35, 0.8);
      side.position.set(cx + s * radius * 0.85, cy + radius * 0.1, cz - radius * 0.1);
      group.add(side);
      const tail = new THREE.Mesh(flatGeometry(new THREE.ConeGeometry(radius * 0.3, radius * 0.9, 4)), hairMat);
      tail.position.set(cx + s * radius * 0.8, cy - radius * 0.55, cz - radius * 0.15);
      tail.rotation.x = -0.9;
      group.add(tail);
    }
  } else if (style === 'short') {
    const top = new THREE.Mesh(flatGeometry(new THREE.SphereGeometry(radius * 0.72, 6, 4)), hairMat);
    top.scale.set(1, 0.6, 1);
    top.position.set(cx, cy + radius * 0.4, cz);
    group.add(top);
  } else if (style === 'hood') {
    const hood = new THREE.Mesh(flatGeometry(new THREE.ConeGeometry(radius * 1.15, radius * 1.6, 6)), hairMat);
